...
function TeamCards() {
    return (
        <div style={{ width: "70%" }}>
            <h4>Team Info:</h4>
            {teamData.map(oneTeam => <TeamCard {...oneTeam}/>) }
        </div>
    );
}

class NCAAHeader extends React.Component {
    render() {
        return (
            <h1>This is a test Website for React! This page will give you some information about the teams competing in March Madness!!!</h1>
        );
    }
}
    ...
    render() {
        return (
            <div style={{width: "20%"}}>
                <h4>Team Names:</h4>
                <ul>
                    {teamData.map(oneTeam => <li>{oneTeam.school}</li>)}
                </ul>
            </div>
        );
    }
